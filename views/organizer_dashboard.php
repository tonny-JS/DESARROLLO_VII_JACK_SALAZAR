...
$stmt = $db->prepare(
    'SELECT COUNT(*) FROM events WHERE organizer_id IN (SELECT id FROM organizers WHERE user_id = :uid)'
);
$stmt->execute([':uid' => $uid]);
$summary['events'] = (int)$stmt->fetchColumn();

$stmt = $db->prepare(
    'SELECT COALESCE(SUM(r.quantity), 0), COALESCE(SUM(r.total_price), 0)
...
$stmt = $db->prepare(
    'SELECT e.id, e.title, e.capacity,
            ...
            COALESCE((SELECT COUNT(*) FROM registrations r WHERE r.event_id = e.id), 0) AS regs
     FROM events e
     WHERE e.organizer_id IN (SELECT id FROM organizers WHERE user_id = :uid)
